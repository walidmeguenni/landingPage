        ...
        <Toolbar component="nav" variant="dense" className={classes.root}>
          ...
          <div className={classes.hide}>
            <div className={classes.link}>
              {NavBarData.map((val, key) => (
                <Link key={key} href={val.link} underline="none" className={ val.link === link ? classes.toolbarLinkActive : classes.toolbarLink } onClick={() => setlink(val.link)} > {val.value} </Link>
              ))}
            </div>
          </div>
          ...
          <div className={classes.hide}>
            <div style={{ display: "flex", flexWrap: "nowrap" }}>
              ...
              <div>
                <Link component={Button} underline="none" variant="outlined" className={classes.buttonHeader} > Singup </Link>
              </div>
            </div>
          </div>
          <Drwer1 />
